...
class Agent():
    ...
    def learn(self):
        ...
        self.optimizer.step()
        # End your code
        torch.save(self.target_net.state_dict(), "./Tables_prep/DQN.pt")
        torch.save(self.evaluate_net.state_dict(), "./Tables_prep/DQNe.pt")
...
def train(env, continue_run):
    ...
    rewards = []
    if continue_run == 1:
        agent.target_net.load_state_dict(torch.load("./Tables_prep/DQN.pt"))
        agent.evaluate_net,load_state_dict(torch.load("./Tables_prep/DQNe.pt"))
        with open('agent_count_prep.txt', 'r') as f:
            c = f.readline()
            agent.count = int(c)
    ...
    for epi in range(episode):
        ...
        while True:
            ...
            with open('agent_count_prep.txt', 'w') as f: 
                f.write(str(agent.count))

            with open('rewards_prep.txt', 'a') as f: 
...
if __name__ == "__main__":
    env = environment()  
    os.makedirs("./Tables_prep", exist_ok=True)
